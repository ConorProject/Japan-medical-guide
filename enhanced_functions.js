...
function generateCleanActionSteps(med) {
    ...
    if (med.status === 'controlled') {
        ...
        if (med.thresholdNumeric && parseFloat(med.thresholdNumeric) > 0 && med.processingDaysMin > 0) {
            // Find the "above threshold" description from the data
            ...
            const aboveThresholdText = higherThresholdMed ? higherThresholdMed.thresholdDescription : `More than ${med.thresholdNumeric}`;
            
            steps.push(`<li class="step-item"><div class="step-marker">2</div><div class="step-text">If ${aboveThresholdText.toLowerCase()}: Apply for Import Confirmation Certificate (${med.processingDaysMin}-${med.processingDaysMax} days)</div></li>`);
