...
function generateCleanActionSteps(med) {
    ...
    if (med.status === 'controlled') {
        ...
        if (med.thresholdNumeric && parseFloat(med.thresholdNumeric) > 0 && med.processingDaysMin > 0) {
            // Extract the actual number from threshold description
            const thresholdValue = parseFloat(med.thresholdNumeric);
            steps.push(`<li class="step-item"><div class="step-marker">2</div><div class="step-text">If bringing more than ${thresholdValue} units: Apply for Import Confirmation Certificate (${med.processingDaysMin}-${med.processingDaysMax} days)</div></li>`);
